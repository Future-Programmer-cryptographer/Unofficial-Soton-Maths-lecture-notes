...
\section{Inhomogeneous ODEs}

We know how to solve these equations and make the dark magic ansatz that seems to work out fine, so I won't be covering the method. 

All my notes will aim to do is to explore how one would come about that ansatz and why it makes sense. 

Inhomogeneous ODEs are in the form of: 

$$
y^{\prime \prime} + p(x) y^{\prime} + q(x)y = r(x) 
$$

Intuition first, inhomogeneous ODEs are all about external forces and having another force in the motion. So the entire problem we're trying to solve is, how does this linear system respond to an external input? 


\subsection{Undetermined coefficients}

If you want to learn how the method works, once again, read up the lecture notes. On the other hand if you'd like some more insight from this person who struggled to gain insight into why this works, read on.. 

In the first instance, this method feels like educated guessing, but it just exploits the property of linear differential operators being a map. 

If the forcing term $r(x)$ has a simple form, then the system responds with the same kind of behaviour. 

Think of it as closure under differentiation. When we differentiate exponentials, we get exponentials, polynomials differentiate to polynomials, and sum of trig functions differentiate to trig functions. 

Now, the other question, why must our trial function be different from the homogeneous solutions. 

Geometrically, differential equations are just vector spaces, and the solutions for our complementary function spans the solution space of the homogeneous operator, so our trial function must be a point out of it. So if our trial function overlaps with the homogeneous space, we just multiply it by $x$ to push it out, which also satisfies the linear independent criteria of the Wronskian. 

\subsection{Variation of Parameters}

Let's just forget the formula for a minute and think about the idea. A homogeneous solution is the form of: 
$$
y_h = c_1y_1 + c_2y_2
$$

These constants are just coordinates of the solution space. So variation of parameters (kinda gives away in the name) is just seeing what happens when we make those coordinates move. 
...
We're essentially seeing what happens to the coefficients with the forcing term involved. 

Another reason to think why this makes sense is because $y_1, y_2$ already span the space of homogeneous solutions, so any solution must be a combination of them. 

Eventually, we end up solving: 
$$
\begin{pmatrix}
    y_1 & y_2 \\
    y^{\prime}_1 & y^{\prime}_2 
\end{pmatrix}
\begin{pmatrix}
    u_1 \\
    u_2
...
This matrix is our basis, and the determinant is the Wronskian again. Homogeneous solutions form a basis \footnote{More on this later in multivariable calculus.  
}. 

\section{Boundary Value Problems}

So far, we have been dealing with initival value problems, or IVPs. Given
$$
y(x_0) = a, \quad y^{\prime}_(x_0) = b
$$

These conditions essentially fix coordinates in a solution space, and always have a unique solution. 

Now we are give some different information: 
$$
y(a) = \alpha, \quad y(b) = \beta
$$

These conditions constrain the solution at two different points and can have either: 
\begin{itemize}
    \item no solutions 
    \item one solution 
...
Geometrically again, all BVPs are doing is telling us how the solution space reacts with the boundary constraints. And the Wronskian appears again here because that tells us whether a basis exists or not. 

Once again, they computational method behind this can be found in the notes with worked examples. 

\section{Eigenvalue Problem}

Recall from Linear Algebra what eigenvalues and eigevectors were (I promise this will be helpful in understanding how and why they work in calculus): 
$$
A\vec{v} = \lambda \vec{v}
$$

Geometrically, this represented the transformation $A$ that sends $\vec{v}$ to a scalar multiple of itself, i.e. the direction of $\vec{v}$ is the same, but its length changes. 

Now, in the world of ODEs, when we write introduce notation like: 
$$
L[y] = \lambda y^{\prime \prime}
$$

where $L$ is a differential operator, this follows exactly the same structure as $Av = \lambda v$, with the main difference being that in linear algebra, $A$ is a matrix whereas in ODEs, $L$ is what we call a differential operator. 

Now, what do we mean by the eigenvalue of an operator? It's just a number $\lambda$ such that the operator acts like a scaling on a function. 
...
The question we are asking ourselves is which functions keep their shape when we differentiate them twice and negate them? 

\subsection{Boundary conditions}

Now, eigenvalue problems differ from normal ODE solving. Normally, we are given initial conditions from which we can always find a unique solution. But now, with the addition of this new eigenvalue, our boundary conditions are constraints on allowed shapes, i.e., for which $\lambda$ does a non trivial solution exist that satisfies the boundary conditions?

\subsection{Solution space}

Now, when we solve something like $-y^{\prime \prime} = \lambda y$ with boundary conditions, $y(0) = 0, y(L) = 0$, we get different behaviours depending on $\lambda$. 

There are three cases for $\lambda: <0, 0, >0$. They are discussed in the lecture notes so I won't go over them. But I will try and give some intuition behind why only positive $\lambda$ works for non-trivial solutions. 

Before I give away the solution, think about it geometrically. Which functions can fit in between the boundary conditions $0$ and $L$ at both ends? A linear function won't, exponentials may explode or decay, so we're left with oscillatory functions. Thinking back to where we get them from, they come from imaginary roots, which we get from $\lambda >0$

So let $\lambda = \mu^2$, this is just another way of writing: $\mu = \sqrt{\lambda}$. Now our equation becomes 
$$
...
$$
y(0) = 0 \implies B = 0 \text{ and } y(L) = 0 \implies \sin(\mu L) = 0
$$
This happens when: 
$$
\mu L = n \pi \implies \mu = \frac{n\pi}{L} \implies \lambda_n = \left( \frac{n\pi}{L}^2 \right )
$$

This also links to the wave function equation, more of which can be foudn in the notes. 


\subsection{Eigenfunctions}

Similar to eigenvectors, the eigenfunctions also form a basis from which we can expand arbitrary functions. 

So instead of vectors in $\mathbb{R^n}$, we have vectors in an infinite dimensional space of functions. 

And this concludes ODEs, and now we move onto more fun stuff in Multivariable Calculus!! 
